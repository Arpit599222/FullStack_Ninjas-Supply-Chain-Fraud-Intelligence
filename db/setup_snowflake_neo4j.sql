...
-- ============================================================
-- PART 1.5: GRAPH PROJECTIONS (REQUIRED FOR NEO4J)
-- ============================================================

-- Node Projections
CREATE OR REPLACE VIEW SELLERS_GRAPH AS
SELECT nodeId, seller_name, platform, city, return_rate, fraud_flag
FROM SELLERS;

CREATE OR REPLACE VIEW WAREHOUSES_GRAPH AS
SELECT nodeId, warehouse_name, city
FROM WAREHOUSES;

-- Relationship Projections (Sellers -> Warehouses)
CREATE OR REPLACE VIEW ORDERS_GRAPH AS
SELECT sourceNodeId, targetNodeId, SUM(order_value) AS weight
FROM ORDERS
GROUP BY sourceNodeId, targetNodeId;

-- Relationship Projections (Sellers sharing same Bank Account)
-- This allows WCC to detect fraud rings
CREATE OR REPLACE VIEW SHARED_BANK_EDGES_GRAPH AS
SELECT 
    s1.nodeId AS sourceNodeId,
    s2.nodeId AS targetNodeId,
    'SHARED_BANK' AS type
FROM SELLERS s1
JOIN SELLERS s2 ON s1.bank_account = s2.bank_account
WHERE s1.nodeId < s2.nodeId; -- Avoid self-loops and duplicates

-- ============================================================
-- PART 2: GRAPH ALGORITHMS (NEO4J NATIVE APP)
-- ============================================================
